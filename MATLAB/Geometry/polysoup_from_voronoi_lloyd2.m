function [polysoup] = polysoup_from_voronoi_lloyd2(x,y,clipping_region,k)
%POLYSOUP_FROM_VORONOI_LLOYD2 Come la funzione polysoup_from_voronoi(),
...
% preservato in modo migliore.
    ...
    if k == 0
        ...
        x = [polysoup.cx; vx(mask)];
        y = [polysoup.cy; vy(mask)];
        C = uniquetol([x,y],'ByRows',true,'DataScale',1);
        x = C(:,1);
        y = C(:,2);
        polysoup = polysoup_from_voronoi(x,y,clipping_region);
