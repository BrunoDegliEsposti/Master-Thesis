function [p_out] = polysoup_clip_convexhull(p_in,clipping_region,mnv_limit)
%POLYSOUP_CLIP_CONVEXHULL Interseca la polysoup p_in con la polyshape
% clipping_region. La clipping region dev'essere interamente contenuta
% nella polysoup.
    ...
    % Calcola l'inviluppo convesso dei poligoni non convessi
    for i = 1:length(psa)
        psxy = psa(i).Vertices;
        if ~ispolyconvex(psxy(:,1),psxy(:,2))
            psa(i) = convhull(psa(i));
        end
    end
