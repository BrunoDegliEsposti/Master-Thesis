function [vertices,edges,cells] = interpolation_linear(vertices,edges,cells)
%INTERPOLATION_LINEAR Ricostruzione lineare dei valori di u sui punti di
% quadratura di ogni spigolo (edges.up e edges.um) a partire dalle medie
% integrali su ogni cella. La ricostruzione viene effettuata componente
% per componente, senza decomposizione caratteristica.
    for i = 1:cells.nc
            ...
            for j = 1:cells.ne(i)
                ...
                if e > 0
                    for k = 1:edges.nq
                        ...
                        edges.up(e,l,k) = p(1) + p(2)*x + p(3)*y;
                    end
                elseif e < 0
                    for k = 1:edges.nq
                        [x,y] = edge_lerp(edges.qx(k),vertices,edges,-e);
                        edges.um(-e,l,k) = p(1) + p(2)*x + p(3)*y;
                    end
                end
            end
        end
