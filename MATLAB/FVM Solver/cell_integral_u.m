function [q_polygon] = cell_integral_u(g,ndim,vertices,edges,cells,i)
%CELL_INTEGRAL_U Calcola l'integrale di una funzione vettoriale g(x,y,ubar)
...
% L'argomento ubar serve a passare a g la media integrale di u
% su ogni cella (cioè il vettore di variabili fisiche cells.u(i,:)).
...
% Questa è anche la formula di quadratura predefinita di FreeFEM.
    ...
    for j = 1:cells.mne
        ...
        for k = 1:length(qw)
            ...
            q_triangle = q_triangle + qw(k)*g(x,y,cells.u(i(mask),:));
        end
